...
while(True):
    ...
    if last_display_time != t:
        ...
        for arr in arrivals:
            ...
            # Ignore any flights that arrived more than 1h ago
            if (atime != 'None' and (t-float(atime)) >= 3600):
                continue

            # Ignore any flights scheduled for more than 1h in the future
            if(stime != 'None' and float(stime) > (t + 3600)):
                continue
            ...
            if etime != 'None' and float(etime) <= (t + ARRIVAL_WARNING_TIME_SECONDS) and flight_id not in announced_arrivals:
                ...
                logfl.flush()
                break
            elif etime != 'None' and float(etime) <= (t + 2*ARRIVAL_WARNING_TIME_SECONDS):
                # Something may be displayed soon so power up
                os.system('uhubctl -l 2 -a 1 > /dev/null')
                time.sleep(1)
            #else:
